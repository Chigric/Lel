...
#define SIZE 	128
#define QUAN_TH 5//quantity locking
...
{//read from shared memory and print to term.
	...
	while(true) {	
		sleep(1);
		pthread_rwlock_rdlock(&locking);
		printf("%x - %s", (unsigned int)pthread_self(), (char*)shmlink);
		pthread_rwlock_unlock(&locking);
	}
...
int main()
{
	...
	int status;//check
	status = pthread_rwlock_init(&locking, NULL);
	if (status != 0) {
		perror("pthread_locking_init");
		return 2;
	}
	...
	status = pthread_rwlock_destroy(&locking);
	if (status != 0) {
		perror("locking_destroy");
		return 2;
	}
